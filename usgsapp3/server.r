...
#   tnrs_prep <- reactive({
#     tnrs(species2(), getpost="POST", source = "NCBI")[,1:5]
#   })
#
...
#     tax_name(query=species2(), get=c("genus", "family", "order", "class", "kingdom"), db="ncbi")
  ...
  output$rank_names <- renderDataTable({
    class_prep()
  })
  ...
  children_prep <- reactive({
    ...
    out <- llply(species2(), function(x) col_downstream(name = x, downto = "Species")[[1]], .parallel=TRUE)
    ldply(out)
  })
...
#  })
  ...
  phylogeny_prep <- reactive({
    ...
    phylog$tip.label <- taxize_capwords(phylog$tip.label)
...
#     print(p)
  ...
  occur_data <- reactive({
    rcharts_prep1(sppchar = input$spec, occurrs = as.numeric(input$numocc), datasource = input$datasource)
...
#     imap = gbifmap2(input = ccc, "MapQuestOpen.OSM")
  ...
  papers_prep <- reactive({
    ...
    dat <- llply(species2(), function(x) searchplos(x, fl='id,journal,title', limit = as.numeric(input$paperlim), key='WQcDSXml2VSWx3P')[,-4])
    names(dat) <- species2()
    dat <- ldply(dat)
    dat$id <- paste0("<a href='http://macrodocs.org/?doi=", dat$id, "' target='_blank'> <i class='fa fa-book'></i> </a>")
